...
def print_metrics(metrics, model_name, class_names=None):
    ...
    # For large datasets, only print top and bottom classes
    num_classes = len(class_names)
    if num_classes > 20:
        print(f"\nTop 10 Classes by F1-Score:")
        top_indices = np.argsort(metrics['f1_per_class'])[-10:][::-1]
        print(f"{'Class':<20} {'Recall':<10} {'Precision':<12} {'F1-Score':<10}")
        print("-" * 55)
        for idx in top_indices:
            print(f"{class_names[idx]:<20} {metrics['per_class_recall'][idx]:<10.4f} "
                  f"{metrics['per_class_precision'][idx]:<12.4f} "
                  f"{metrics['f1_per_class'][idx]:<10.4f}")
        
        print(f"\nBottom 10 Classes by F1-Score:")
        bottom_indices = np.argsort(metrics['f1_per_class'])[:10]
        print(f"{'Class':<20} {'Recall':<10} {'Precision':<12} {'F1-Score':<10}")
        print("-" * 55)
...
def plot_confusion_matrix(cm, class_names, save_path, model_name):
    ...
    # For large datasets, use larger figure size and smaller font
    if num_classes > 50:
        figsize = (30, 28)
        fontsize = 6
        annot = False  # Don't annotate cells for very large matrices
    elif num_classes > 20:
        figsize = (20, 18)
        fontsize = 8
        annot = True
...
def plot_per_class_metrics(metrics_cnn, metrics_vit, class_names, save_path):
    ...
    # For large datasets, use vertical layout and smaller bars
    if num_classes > 50:
        figsize = (20, 12)
        width = 0.3
        rotation = 90
    ...
    axes[0].set_title('Per-Class Recall', fontsize=12)
    axes[0].set_xticks(x)
    axes[0].set_xticklabels(class_names, rotation=rotation, ha='right', fontsize=fontsize)
    axes[0].legend()
    ...
    axes[1].set_title('Per-Class Precision', fontsize=12)
    axes[1].set_xticks(x)
    axes[1].set_xticklabels(class_names, rotation=rotation, ha='right', fontsize=fontsize)
    axes[1].legend()
    ...
    axes[2].set_title('Per-Class F1-Score', fontsize=12)
    axes[2].set_xticks(x)
    axes[2].set_xticklabels(class_names, rotation=rotation, ha='right', fontsize=fontsize)
    axes[2].legend()
